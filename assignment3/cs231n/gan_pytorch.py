...
def discriminator_loss(logits_real, logits_fake):
    ...
    l1=bce_loss(logits_real,torch.ones(logits_real.size()[0]).type(dtype))
    l2=bce_loss(logits_fake,torch.zeros(logits_fake.size()[0]).type(dtype))
    loss=l1+l2
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    return loss
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    a1=(scores_real-1)**2
    l1=1/2*a1.sum()/a1.size()[0]

    a2=scores_fake**2
    l2=1/2*a2.sum()/a2.size()[0]
