...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        e=torch.arange(0,embed_dim,2)
        o=e+1
        p1=torch.arange(0,max_len).view(-1,1)*10000**(-e/embed_dim)
        p2=torch.arange(0,max_len).view(-1,1)*10000**(-(o-1)/embed_dim)
        pe[0,:,e]=torch.sin(p1)
        pe[0,:,o]=torch.cos(p2)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        if attn_mask!=None:
          s=s.masked_fill(attn_mask==0,-math.inf)
        s=self.attn_drop((F.softmax(s,-1)))
        s=torch.matmul(s,v)
        output=self.proj(s.transpose(1,2).reshape(N, S, -1))
